      ...
      <div className="max-w-2xl mx-auto mt-8 px-4">
        <div className="bg-white rounded-2xl shadow-2xl overflow-hidden backdrop-blur-lg backdrop-filter border border-gray-100">
          ...
          <div className="h-[500px] overflow-y-auto p-4 space-y-4 bg-gradient-to-b from-gray-50 to-white">
            {error && (
              <div className="bg-red-50 text-red-600 p-3 rounded-xl border border-red-100 text-center text-sm animate-fade-in">
                {error}
              </div>
            )}
            ...
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-fade-in-up`}>
                <div className={`max-w-[70%] p-3 rounded-2xl shadow-sm ${msg.role === 'user'
                  ...
                  <ReactMarkdown
                    ...
                    components={{
                      ...
                      a: ({ children, href }) => (
                        <a href={href} className={`underline ${msg.role === 'user' ? 'text-blue-100' : 'text-blue-600'} hover:opacity-80`} target="_blank" rel="noopener noreferrer">
                          {children}
                        </a>
                      ),
                      code: ({ children }) => (
          ...
          <div className="p-4 bg-gradient-to-b from-white to-gray-50 border-t border-gray-100">
            <div className="flex space-x-2">
              <input
                type="text"
                className="flex-1 px-4 py-2 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
              ...
              <button
                ...
                className="px-6 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:opacity-90 text-sm font-medium shadow-lg disabled:opacity-50"
              >
                Send
              </button>
            </div>
